...
create function public.post_journal_entry(target_entry_id uuid)
returns void
language plpgsql
security invoker
set search_path = ''
...
begin
  ...
  into target_company_id, target_status
  ...
  if not public.is_company_member(target_company_id) then
    raise exception 'Journal entry belongs to another company' using errcode = '42501';
  end if;
  if not (
    public.user_has_permission(target_company_id, 'journal_entries.post')
    or public.user_has_permission(target_company_id, 'accounting.manage')
    or public.user_has_permission(target_company_id, 'invoices.post')
    or public.user_has_permission(target_company_id, 'payments.create')
    or public.user_has_permission(target_company_id, 'expenses.approve')
  ) then
    raise exception 'Journal posting permission required' using errcode = '42501';
  end if;
  if target_status = 'posted' then return; end if;
  ...
  if debit_total <= 0 or credit_total <= 0 or debit_total <> credit_total then
    raise exception 'Journal entry is empty or unbalanced';
  end if;

  update public.journal_entries
  set status = 'posted', posted_at = now()
  where id = target_entry_id and company_id = target_company_id and status = 'draft';
end;
$$;
...
grant execute on function public.post_invoice_transaction(uuid) to authenticated;
...
revoke execute on function public.post_invoice_transaction(uuid) from anon, public;
...
revoke execute on function public.post_expense_transaction(uuid) from anon, public;

drop policy if exists "journal_entries accounting insert" on public.journal_entries;
drop policy if exists "journal_entries accounting update" on public.journal_entries;
drop policy if exists "journal_entry_lines accounting insert" on public.journal_entry_lines;
drop policy if exists "payment_allocations accounting insert" on public.payment_allocations;
drop policy if exists "customer_ledger accounting insert" on public.customer_ledger;

create policy "journal entries authorized insert" on public.journal_entries for insert to authenticated with check (
  created_by = auth.uid() and public.is_company_member(company_id) and (
    public.user_has_permission(company_id,'accounting.manage') or public.user_has_permission(company_id,'journal_entries.create') or public.user_has_permission(company_id,'invoices.post') or public.user_has_permission(company_id,'payments.create') or public.user_has_permission(company_id,'expenses.approve')));
create policy "journal entries authorized update" on public.journal_entries for update to authenticated using (
  public.is_company_member(company_id) and (public.user_has_permission(company_id,'accounting.manage') or public.user_has_permission(company_id,'journal_entries.post') or public.user_has_permission(company_id,'invoices.post') or public.user_has_permission(company_id,'payments.create') or public.user_has_permission(company_id,'expenses.approve')))
with check (public.is_company_member(company_id));
create policy "journal lines authorized insert" on public.journal_entry_lines for insert to authenticated with check (
  created_by = auth.uid() and public.is_company_member(company_id) and (public.user_has_permission(company_id,'accounting.manage') or public.user_has_permission(company_id,'journal_entries.create') or public.user_has_permission(company_id,'invoices.post') or public.user_has_permission(company_id,'payments.create') or public.user_has_permission(company_id,'expenses.approve')));
create policy "payment allocations authorized insert" on public.payment_allocations for insert to authenticated with check (
  created_by = auth.uid() and public.is_company_member(company_id) and (public.user_has_permission(company_id,'accounting.manage') or public.user_has_permission(company_id,'payments.create')));
create policy "customer ledger authorized insert" on public.customer_ledger for insert to authenticated with check (
  created_by = auth.uid() and public.is_company_member(company_id) and (public.user_has_permission(company_id,'accounting.manage') or public.user_has_permission(company_id,'invoices.post')));
